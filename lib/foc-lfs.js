...
const resolveLfs = async uploadKey => {
...
export const downloadLfs = async (synapse, uploadKey) => {
  const pieceCids = await resolveLfs(uploadKey)
...
export const deleteLfs = async (synapse, uploadKey) => {
  const pieceCids = await resolveLfs(uploadKey)
  await Promise.all(pieceCids.map(pieceCid =>
    synapse.storage.deletePiece({ piece: pieceCid }))
  )
}
